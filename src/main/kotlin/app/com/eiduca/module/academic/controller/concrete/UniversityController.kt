...
import app.com.eiduca.module.academic.request.UniversityRequest
...
class UniversityController(
...
): ConcreteController<University, UniversityRequest>() {
    ...
    override fun findAll(@ParameterObject pageable: Pageable): ResponseEntity<Page<University>> {
        return ResponseEntity(universityService.findAll(pageable), ReturnStatus.OK)
    }
    ...
    @ApiResponses(value = [ ApiResponse(responseCode = MessageDoc.STATUS_PERMISSION_DENIED, description = MessageDoc.PERMISSION_DENIED) ])
    override fun findById(@PathVariable id: String): ResponseEntity<University> {
        return ResponseEntity(universityService.findById(id), ReturnStatus.OK)
    }
    ...
    override fun save(@Valid @RequestBody request: UniversityRequest): ResponseEntity<University> {
        return ResponseEntity(universityService.save(request.toModel()), ReturnStatus.CREATED)
    }
    ...
    override fun update(@Valid @RequestBody request: UniversityRequest, @PathVariable id: String): ResponseEntity<University> {
        return ResponseEntity(universityService.update(request.toModel(),id), ReturnStatus.UPDATED)
    }
    ...
    override fun deleteById(@PathVariable id: String): ResponseEntity<Unit> {
        return ResponseEntity(universityService.deleteById(id), ReturnStatus.DELETED)
    }
}